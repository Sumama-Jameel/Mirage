...
## Grok provider challenge constants
...
## Minimax Provider — 2026-07-17 breakthrough
...
**Agent names:** Agents have NUMERIC string IDs from `GET /archon/api/v1/agent`. The `userID` in `user_detail_agent` is for the user-as-creator, not the agent-to-chat-with. The 5 built-in agents: `405561744400588` (Mavis, default), `405561744400589` (General), `405561744400590` (Coder), `405561744400591` (Verifier), `405561744400587`.

**SSE format:** New archon API uses `type:6` events with `agent_message_chunk` (NOT the old `messages[]`/`is_final` format). Each chunk carries FULL content; client computes delta via prefix matching.

**Message body:** `turn_id` and `worktreeMode` must be JSON strings (not numbers/booleans), `variant` must be `""` not `null`.
