...
pub struct BrowserContext {
    ...
    pub stealth: bool,
    /// Browser-family identity exposed to the JS runtime: "chrome" or "firefox".
    pub identity: String,
...
impl BrowserContext {
    ...
    fn _new_inner(
        ...
        // When a custom UA is supplied, derive platform/identity from it so the
        // headless engine impersonates that browser consistently. Otherwise use
        // the built-in Chrome rotation.
        let (resolved_ua, platform, ua_platform, ua_platform_version, identity) = match user_agent {
            Some(ref ua) => {
                let id = crate::profiles::identity_from_ua(ua);
                (
                    id.user_agent,
                    id.platform,
                    id.ua_platform,
                    id.ua_platform_version,
                    crate::profiles::browser_family_from_ua(ua).to_string(),
                )
            }
            None => {
                let p = crate::profiles::select_profile();
                (
                    p.user_agent.to_string(),
                    p.platform.to_string(),
                    p.ua_platform.to_string(),
                    p.ua_platform_version.to_string(),
                    "chrome".to_string(),
                )
            }
        };
