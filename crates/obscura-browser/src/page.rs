...
use obscura_dom::{ax, parse_html, AXNode, DomTree, NodeId};
...
impl Page {
    ...
    fn init_js(&mut self) {
        ...
        if self.stealth_client.is_some() {
            rt.set_stealth(true);
            rt.set_identity(&self.context.identity);
    ...
    ///
    /// Returns a flat list of [`AXNode`] entries, one per DOM node with a
    /// non-empty role, in document order. Used by gateway adapters that
    /// extract visible text from the rendered page rather than intercepting
    /// network responses.
    ///
    /// Returns an empty Vec when the page has no DOM yet (e.g. before the
    /// first navigation).
    pub fn ax_tree(&self) -> Vec<AXNode> {
        self.with_dom(|dom| ax::ax_tree(dom)).unwrap_or_default()
    }

    /// Extract the visible text content of a subtree rooted at `root`.
    ///
    /// This is the primary text-extraction primitive for gateway adapters:
    /// it walks the DOM, skips author-hidden subtrees and non-content roles
    /// (buttons, images, form controls, decorative elements), and joins
    /// the remaining text with proper block-level whitespace separation.
    ///
    /// Returns an empty string when the root has no DOM or the subtree is
    /// entirely hidden. Pure DOM walk — no layout, no JS execution.
    pub fn visible_text(&self, root: NodeId) -> String {
        self.with_dom(|dom| ax::visible_text(dom, root))
            .unwrap_or_default()
    }

    /// Query a CSS selector and return the first matching node, if it is
    /// **visible** per author intent.
    ///
    /// Unlike [`DomTree::query_selector`] (which returns the first match
    /// regardless of hidden state), this filters out nodes that the author
    /// marked as hidden via attributes, inline style, or known utility
    /// classes. If the first matching node is hidden, it walks subsequent
    /// matches until it finds a visible one.
    ///
    /// Returns:
    /// - `Ok(None)` when the selector matches no visible node.
    /// - `Err(String)` when the selector itself fails to parse.
    pub fn query_visible(&self, selector: &str) -> Result<Option<NodeId>, String> {
        self.with_dom(|dom| {
            // Collect all matches; query_selector returns only the first.
            let matches = dom.query_selector_all(selector)?;
            for nid in matches {
                if !ax::node_is_hidden_in(dom, nid) {
                    return Ok(Some(nid));
                }
            }
            Ok(None)
...
mod tests {
    ...
    /// Helper: build a Page with a pre-populated DOM for unit-testing the
    /// accessibility-aware methods without spinning up the full lifecycle.
    fn page_with_dom(html: &str) -> Page {
        let context = Arc::new(BrowserContext::new("test-ctx".to_string()));
        let mut page = Page::new("test-page".to_string(), context);
        page.dom = Some(parse_html(html));
        page
    }

    #[test]
    fn page_visible_text_basic() {
        let page = page_with_dom("<html><body><p>Hello world</p></body></html>");
        let body = page.dom().unwrap().find_body_or_root();
        let text = page.visible_text(body);
        assert_eq!(text, "Hello world");
    }

    #[test]
    fn page_visible_text_skips_hidden_subtree() {
        let page = page_with_dom(
            "<html><body><div><span hidden>secret</span>visible</div></body></html>",
        );
        let body = page.dom().unwrap().find_body_or_root();
        assert_eq!(page.visible_text(body), "visible");
    }

    #[test]
    fn page_visible_text_skips_button() {
        let page = page_with_dom(
            "<html><body><div>before <button>Send</button> after</div></body></html>",
        );
        let body = page.dom().unwrap().find_body_or_root();
        // Button is skipped; the surrounding spaces remain (one before the
        // button, one after) so the result has two spaces.
        assert_eq!(page.visible_text(body), "before  after");
    }

    #[test]
    fn page_visible_text_empty_when_dom_missing() {
        let context = Arc::new(BrowserContext::new("test-ctx".to_string()));
        let mut page = Page::new("empty-page".to_string(), context);
        page.dom = None;
        let text = page.visible_text(obscura_dom::NodeId::new(0));
        assert_eq!(text, "");
    }

    #[test]
    fn page_ax_tree_returns_root_and_text() {
        let page = page_with_dom("<html><body><p>hi</p></body></html>");
        let tree = page.ax_tree();
        assert!(!tree.is_empty());
        assert!(tree.iter().any(|n| n.role == "RootWebArea"));
        assert!(tree.iter().any(|n| n.role == "StaticText"));
    }

    #[test]
    fn page_ax_tree_empty_when_no_dom() {
        let context = Arc::new(BrowserContext::new("test-ctx".to_string()));
        let mut page = Page::new("empty-page".to_string(), context);
        page.dom = None;
        assert!(page.ax_tree().is_empty());
    }

    #[test]
    fn page_query_visible_returns_visible_match() {
        let page = page_with_dom(
            "<html><body><p id=\"msg\">visible text</p></body></html>",
        );
        let nid = page.query_visible("#msg").expect("selector parses");
        assert!(nid.is_some(), "visible element must be returned");
    }

    #[test]
    fn page_query_visible_skips_hidden_first_match() {
        // Two <p> elements: first is hidden, second is visible. query_visible
        // must walk past the hidden match and return the second.
        let page = page_with_dom(
            "<html><body><p id=\"a\" hidden>nope</p><p id=\"b\">yes</p></body></html>",
        );
        let nid = page.query_visible("p").expect("selector parses").unwrap();
        let node = page.dom().unwrap().get_node(nid).unwrap();
        let id_attr = match &node.data {
            obscura_dom::NodeData::Element { attrs, .. } => attrs
                .iter()
                .find(|a| a.name.local.as_ref() == "id")
                .map(|a| a.value.clone()),
            _ => None,
        };
        assert_eq!(id_attr.as_deref(), Some("b"));
    }

    #[test]
    fn page_query_visible_no_match_returns_none() {
        let page = page_with_dom("<html><body><p>x</p></body></html>");
        let nid = page.query_visible(".does-not-exist").expect("selector parses");
        assert!(nid.is_none());
    }

    #[test]
    fn page_query_visible_invalid_selector_errors() {
        let page = page_with_dom("<html><body></body></html>");
        assert!(page.query_visible("[[[").is_err());
    }
}
