...
globalThis.navigator = {
  ...
  get appVersion() {
    if (globalThis.__obscura_identity === 'firefox') {
      var plat = globalThis.__obscura_platform || 'Linux x86_64';
      if (plat.indexOf('Linux') >= 0) return '5.0 (X11)';
      if (plat.indexOf('Mac') >= 0) return '5.0 (Macintosh)';
      return '5.0 (Windows)';
    }
  ...
  maxTouchPoints: 0,
  get vendor() { return globalThis.__obscura_identity === 'firefox' ? '' : 'Google Inc.'; },
  product: "Gecko",
  get productSub() { return globalThis.__obscura_identity === 'firefox' ? '20100101' : '20030107'; },
  get oscpu() { return globalThis.__obscura_identity === 'firefox' ? (globalThis.__obscura_platform || 'Linux x86_64') : undefined; },
  get buildID() { return globalThis.__obscura_identity === 'firefox' ? '20240627000000' : undefined; },
  doNotTrack: null,
  deviceMemory: 8,
...
globalThis.__obscura_init = function() {
  ...
  // Default to Chrome identity if the caller did not set one. The runtime
  // normally calls set_identity before __obscura_init, but defaults keep old
  // behavior intact.
  if (!globalThis.__obscura_identity) {
    globalThis.__obscura_identity = 'chrome';
  }
  // The chrome global only exists on Chrome-family browsers.
  if (globalThis.__obscura_identity === 'chrome') {
    _installChromeGlobal();
  }
